...
class BusStopSeeder extends Seeder
{
    public function run()
    {
        ...
        $this->createBusesWithStops($route11_forward, ['Цирк', 'ул. Некрасова', 'Театр драмы', 'Сквер Лермонтова', 'Университет', 'ул. Ленина', 'ул. Маяковского', 'ул. Пушкина', 'Театр оперы', 'ул. Попова'], '06:45', 30);
        $this->createBusesWithStops($route11_backward, ['ул. Попова', 'Театр оперы', 'ул. Мельника', 'Завод ТочМаш', 'ул. Бунина', 'ТРЦ Волна', 'ул. Университетская', 'ул. Некрасова', 'Цирк'], '06:30', 30);

        // Добавляем автобусы и остановки для маршрута 21 (вперед и назад)
        $this->createBusesWithStops($route21_forward, ['ул. Пушкина', 'ул. Лыскова', 'Университет', 'Сквер Лермонтова', 'Театр драмы', 'ул. Некрасова', 'Цирк', 'Театр Миниатюры', 'ул. Чехова', 'ул. Ленина'], '06:15', 40);
        $this->createBusesWithStops($route21_backward, ['ул. Ленина', 'ул. Чехова', 'Театр Миниатюры', 'Цирк', 'ул. Некрасова', 'Театр драмы', 'Сквер Лермонтова', 'Университет', 'ул. Лыскова', 'ул. Пушкина'], '06:00', 40);
    }
    ...
    private function createBusesWithStops($route, $stops, $startTime, $interval)
    {
        ...
        for ($busIndex = 0; $busIndex < 20; $busIndex++) {
            // Создаем автобус с уникальным именем
          //  $bus = Bus::firstOrCreate(['name' => "Автобус №" . ($busIndex + 1) . " " .  $route->name, 'route_id' => $route->id]);
            $bus = Bus::firstOrCreate(['name' => "Автобус №" . ($busIndex + 1), 'route_id' => $route->id]);
